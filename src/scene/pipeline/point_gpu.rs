...
struct StyleUniforms {
    ...
    _pad2: [f32; 2],
    // Cross-section band: p0.xy, p1.xy, and (width_px, mode). mode: 0 =
    // off, 1 = dim, 2 = discard. `width_px` is the total screen width (the
...
impl StyleUniforms {
    fn new(model: &PointCloudModel) -> Self {
        ...
        let (p0, p1, params) = match style.section {
            Some(section) if section.p0 != section.p1 && section.width_px > 0.0 => (
                [section.p0[0] as f32, section.p0[1] as f32],
                [section.p1[0] as f32, section.p1[1] as f32],
                [
                    section.width_px as f32,
                    match section.mode {
