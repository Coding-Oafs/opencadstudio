...
struct Style {
    ...
    section_p1: vec2<f32>,
    _pad4: vec2<f32>,
    section_params: vec2<f32>, // (half_width_px, mode: 0=off 1=dim 2=discard)
    _pad5: vec2<f32>,
...
fn section_outside(position: vec2<f32>) -> f32 {
    ...
    // The band is authored in screen pixels; scale to world units so the slice
    // keeps a fixed on-screen width as the camera zooms.
    let half = style.section_params.x * u.world_per_pixel;
    if (dist <= half) {
        return 0.0;
    }
