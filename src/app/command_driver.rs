        ...
        match result {
            ...
            CmdResult::ReassociateCenterMark {
                    ...
                    .reassociate_center_mark(target, source, point)
                {
                    self.tabs[i].dirty = true;
                    self.command_line
                        .push_output("CENTERREASSOCIATE: center mark associated.");
                } else {
                    self.command_line
                        .push_error("CENTERREASSOCIATE: the selected source is not circular.");
                }
                self.tabs[i].active_cmd = None;
                self.tabs[i].snap_result = None;
                self.tabs[i].scene.clear_preview_wire();
                ...
                if new_entities.len() == 1 {
                    if let acadrust::EntityType::XLine(ref xl) = new_entities[0] {
                            ...
                            || layer.starts_with("__DIMBREAK_AUTO__")
                        {
                            ...
                            self.command_line.push_info(
                                crate::t!("DIMBREAK: not yet implemented — nothing changed.")
                                    .as_ref(),
                            );
                            self.tabs[i].active_cmd = None;
                            self.tabs[i].snap_result = None;
                        ...
                        if layer.starts_with("__DIMJOG__") {
                            ...
                            self.command_line.push_info(
                                crate::t!("DIMJOGLINE: not yet implemented — nothing changed.")
                                    .as_ref(),
                            );
                            self.tabs[i].active_cmd = None;
                            self.tabs[i].snap_result = None;
            ...
            CmdResult::MeasureEntity {
                ...
                if count > 0 {
                    ...
                    self.command_line.push_error(
                        crate::t!("MEASURE: entity type not supported or distance too large.")
                            .as_ref(),
                    );
                }
                self.tabs[i].active_cmd = None;
                ...
                match &op {
                    ...
                    _ => {
                        ...
                        if changed {
                            ...
                            self.discard_last_undo_entry(i);
                            self.command_line.push_error(
                                crate::t!("PEDIT: operation not applicable to this entity.")
                                    .as_ref(),
                            );
                        }
                    }
                    ...
                    if let Some(mesh) = mesh_fn(name) {
                            ...
                            self.tabs[i].scene.document.get_entity_mut(handle)
                        {
                            ...
                            entity.point_of_reference =
                                acadrust::types::Vector3::new(center[0], center[1], center[2]);
                        }
                        self.tabs[i].scene.meshes.insert(handle, set);
                    }
                    self.tabs[i].dirty = true;
                    self.command_line
                        .push_output(crate::t!("Solid created.").as_ref());
                }
                if let Some(pd) = pending {
                    self.commit_undo_delta(i, pd);
                }
            ...
            CmdResult::ExtrudeEntity {
                ...
                if let Some(entity) = entity_opt {
                    ...
                    if let Some((mesh, solid)) = result {
                        ...
                        self.tabs[i].scene.create_solid_history(new_handle, history);
                    ...
                    self.command_line
                        .push_error(crate::t!("EXTRUDE: entity not found.").as_ref());
                }
                self.tabs[i].active_cmd = None;
                self.tabs[i].snap_result = None;
                self.tabs[i].scene.clear_preview_wire();
            ...
            CmdResult::LoftEntities { handles, color } => {
                ...
                if let Some(mut set) = sweep_model::lofted(&profiles, color) {
                    ...
                    self.command_line.push_error(
                        crate::t!("LOFT: select at least two closed profiles.").as_ref(),
                    );
                }
                self.tabs[i].active_cmd = None;
                self.tabs[i].snap_result = None;
